...
"""
        descricoes = "\n".join([f"DESCRICAO {i+1}: {desc}" for i, desc in enumerate(descricoes_batch)])
...
class EmbeddingLocalOtimizado:
    ...
    def criar_texto_embedding(self, row: Dict, metadados: Dict) -> str:
        ...
        return f"Categoria: {categoria} | Subcategoria: {subcategoria} | Marca: {marca} | Modelo: {modelo} | DESCRICAO: {descricao}"
...
class IndexadorOtimizado:
    ...
    def processar_lote(self, dados: pd.DataFrame, batch_size: int = 20) -> List[Dict]:
        ...
        for i in range(0, len(dados), batch_size):
            ...
            descricoes = batch['DESCRICAO'].tolist()
