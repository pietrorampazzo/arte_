    ...
    def indexar_produtos(self, caminho_dados, incremental=False):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                # Processar descrição
                metadados = self.processador.extrair_specs_com_llm(row['Descrição'], row_index=idx)
                embedding = self.processador.criar_embedding(metadados, row['Descrição'], row_index=idx)
                ...
                novos_produtos.append((
                    idx,
                    str(row['Descrição']) if not pd.isna(row['Descrição']) else "",
