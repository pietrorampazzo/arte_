...
CAMINHO_BASE = r"saida_unificada/produtos_categorizados_v4.xlsx"
...
Item nº {item_data['Nº']}
Descrição: {item_data['DESCRICAO']}
Unidade: {item_data['UNID_FORN']}
Quantidade: {item_data['QTDE']}
Valor Unitário Ref.: {item_data['VALOR_UNIT']}
Valor Total Ref.: {item_data['VALOR_TOTAL']}
Local Entrega: {item_data['LOCAL_ENTREGA']}
</item_edital>
...
for idx, row in df_edital.iterrows():
    ...
    if df_filtrado.empty:
        ...
        base_json = df_filtrado[['categoria_principal', 'subcategoria', 'MARCA', 'MODELO', 'VALOR', 'DESCRICAO']].to_json(orient="records", force_ascii=False, indent=2)

        # Passo: Selecionar melhor via LLM
        colunas = selecionar_melhor(model, item_data, base_json)
    time.sleep(60)  # Pausa para API
