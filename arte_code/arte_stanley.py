...
CAMINHO_SAIDA = os.path.join(BASE_DIR, "DOWNLOADS", "master_stanley.xlsx")
...
def check_match_found(ai_result):
    """Verifica se o resultado da IA contém um 'best_match' com 95% ou mais de compatibilidade."""
    best_match_data = ai_result.get("best_match")
    if not best_match_data:
        return False

    analise = best_match_data.get('Compatibilidade_analise', '')
    # Tenta extrair uma porcentagem ou uma contagem "X de Y"
    match_percent_direct = re.search(r'(\d{2,3})\s?%', analise)
    if match_percent_direct:
        if int(match_percent_direct.group(1)) >= 95:
            return True

    match_num = re.search(r'atende a (\d+) de (\d+)', analise, re.IGNORECASE)
    if match_num:
        attended, total = map(int, match_num.groups())
        if total > 0 and (attended / total) >= 0.95:
            return True

    # Se a análise for positiva mas sem números, consideramos um match.
    if "compatível" in analise.lower() and "parcial" not in analise.lower() and "falha em" not in analise.lower():
        return True
...
def calculate_compatibility_score(analise: str, edital_subcategory: str | None, product_subcategory: str | None) -> float:
    ...
    if analise and pd.notna(analise):
        ...
        if match_num:
            ...
            failed_count = 0
            if atende_match:
                attended_count = len(re.findall(r'[^,;.]+', atende_match.group(1).strip()))
            if falha_match:
                failed_count = len(re.findall(r'[^,;.]+', falha_match.group(1).strip()))
...
def main():
    ...
    for idx, item_edital in df_edital_new.iterrows():
        ...
        if data_to_populate:
            ...
            if desc_fornecedor:
                matched_rows = df_base[df_base['DESCRICAO'] == desc_fornecedor]
                if not matched_rows.empty:
                    product_subcategory = matched_rows.iloc[0]['subcategoria']

            compat_score = calculate_compatibility_score(analise_compat, edital_subcategory, product_subcategory)
