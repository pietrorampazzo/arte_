...
def gerar_conteudo_com_fallback(client: Groq, prompt: str, modelos: list[str]) -> str | None:
    """
    ...
    for nome_modelo in modelos:
        try:
            ...
            if not response_content.strip():
                 print(f"   - ❌ A GERAÇÃO RETORNOU VAZIA.")
                 return None
...
def get_item_classification(client: Groq, description: str, reference: str, categories_with_subcategories: dict) -> dict | None:
...
'''
    ...
    response_text = gerar_conteudo_com_fallback(client, prompt, LLM_MODELS_FALLBACK)
    if response_text:
        try:
...
def get_best_match_from_ai(client: Groq, item_edital, df_candidates):
...
def get_top_5_matches_by_category(client: Groq, item_edital, df_candidates, category_type):
    """
    2nd and 3rd LLM:
...
def get_best_match_from_combined(client: Groq, item_edital, combined_products):
    """
    4th LLM:
...
def main():
    ...
    if not api_key:
        logger.error("QROQ_API_KEY not found in .env file.")
        print("ERROR: QROQ_API_KEY not found in .env file.")
        return
    ...
    for idx, item_edital in df_edital_new.iterrows():
        ...
        if df_candidates.empty:
            ...
            classification = get_item_classification(client, descricao, referencia, CATEGORIZATION_KEYWORDS)
        ...
        if df_final_candidates.empty:
            ...
            ai_result = get_best_match_from_ai(client, item_edital, df_final_candidates)
            time.sleep(30)
                ...
                if match_percent is not None and match_percent >= 95:
                    ...
                    if classification and classification.get('categoria_principal'):
                        ...
                        if not df_main_cat.empty:
                            top5_main = get_top_5_matches_by_category(client, item_edital, df_main_cat, 'categoria_principal')
                    ...
                    if classification and classification.get('subcategoria'):
                        ...
                        if not df_sub_cat.empty:
                            top5_sub = get_top_5_matches_by_category(client, item_edital, df_sub_cat, 'subcategoria')

                    combined_candidates = top5_main + top5_sub

                    if combined_candidates:
                        best_combined = get_best_match_from_combined(client, item_edital, combined_candidates)
                        if best_combined:
