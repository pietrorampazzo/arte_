...
def main():
    ...
    for idx, item_edital in df_edital_new.iterrows():
        ...
        if valor_unit_edital > 0:
            ...
            print("- ℹ Valor de referência do edital é R$0.00 ou inválido. Analisando todos os produtos da base.")
            df_candidates = df_base.copy()

        if df_candidates.empty:
            if valor_unit_edital > 0:
                print(f"- ⚠ Nenhum produto encontrado abaixo do custo máximo de R${max_cost:.2f}.")
            ...
            if df_final_candidates.empty:
                print(f"- ⚠ Nenhum produto encontrado na categoria '{item_category}'.")
    ...
    if not results:
        print("- ⚠ No new results were generated to add.")
        return
