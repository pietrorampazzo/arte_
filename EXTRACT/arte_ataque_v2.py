...
def extrair_itens_pdf_texto(text):
    ...
    for i, match in enumerate(item_matches):
        ...
        valor_unitario_match = re.search(r'Valor Unitário[^:]*:\s*R?$?$?s?\s*([\d.,]+)', item_text, re.IGNORECASE)
        valor_unitario = valor_unitario_match.group(1) if valor_unitario_match else ""

        valor_total_match = re.search(r'Valor Total[^:]*:\s*R?$?$?s?\s*([\d.,]+)', item_text, re.IGNORECASE)
